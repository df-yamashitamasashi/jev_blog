...
| [`python/evaluation/`](python/evaluation/README.md) | 本物の Jev / Gemini で行った評価（質問68問＋改善後の追加10問）の手順・データ・結果 |
| `jev-rag-workbench/` | 動作の可視化。利用者・Jev・検索・LLM の誰が何をしたかを、フロー図で表示する。評価の記録を API キーなしで再生できる |
| `images/` | 記事で使っているスクリーンショット |

### 評価結果（jev-1.13.0 + gemini-3.8-flash、評価60問。評価後の改善2点を反映）
- 答えのない質問 20/20 を、LLM に渡さずに止めた
- 答えのある質問 34問中33問に正しく回答
- 文書にない内容を含む文 20/20 を検出、正しい文を誤って疑ったのは 0/20
- LLM の呼び出し 60回→33回、LLM のトークン −65%
...
### Results (jev-1.13.0 + gemini-3.8-flash, 60 test questions, with the two post-evaluation fixes)
- Stopped 20/20 unanswerable questions before calling the LLM
- Answered 33 of 34 answerable questions correctly
- Flagged 20/20 unsupported sentences, with 0/20 false alarms on supported ones
- LLM calls 60 → 33, LLM tokens −65%
