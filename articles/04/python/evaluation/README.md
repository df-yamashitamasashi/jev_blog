...
上の評価（v1）は、コミット `25b8925` の状態で実行したものです。評価後に次の変更を加えたため、`jev_rag/config.py` と `jev_rag/pipeline.py` は上のハッシュ値と一致しません。

### 改善（v2）
評価で見つかった2つの失敗に対する改善です。どちらも、評価データを見てから決めた変更です。

| 変更 | 理由（評価データ） |
| :--- | :--- |
| 「挨拶」「曖昧」判定の確信度が0.5未満なら、文書を調べる | 正しい判定はすべて確信度0.99以上。誤って聞き返した t28 は0.27 |
| Gate 5 の基準を 0.85 → 0.80 に下げる | 正しい文への誤警告4件のうち3件が0.82〜0.84。根拠のない文は最大0.25 |

`run_v2.py` で、次の2つを行いました。

1. **評価済みの質問への反映**（`results/test_v2/`, `results/dev_v2/`）：記録済みの Jev の確率に v2 の規則を当てはめて再計算しました。判定の流れが変わる t28 だけは、実際に実行し直しています。
   - 答えのある質問の正答：32/34 → 33/34
   - 実際の回答文への誤警告：4 → 1
   - 答えのない質問を止めた数：20/20 のまま
2. **未使用の10問での確認**（`holdout.jsonl` → `results/holdout/`）：v2 を決めたあとに作った質問で、1回だけ実行しました。
   - 答えのある5問中4問に正しく回答
   - 答えのない2問には回答せず（1問は「見つかりません」、1問は聞き返し）
   - 挨拶・曖昧な入力の3問は正しく処理
   - 「インボイス番号」は確信度0.71で「曖昧」と判定され、聞き返しのままでした。単語だけの質問は v2 でも改善しません。

### そのほかの追加（評価結果には影響しない）
- `export_replays.py`：記録を Workbench の「実測リプレイ」用に書き出すスクリプトです（API は使いません）。調整用（dev）の記録は保存済みの応答から再構成したものなので、処理時間は書き出していません。
